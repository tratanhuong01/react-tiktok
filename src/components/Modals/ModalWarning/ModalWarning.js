...
export default function ModalWarning(props) {
    //
    const { title, description, buttonFirst, buttonSecond, functionButtonFirst, functionButtonSecond,
        paramButtonFirst, paramButtonSecond } = props.data;
    ...
    return (
        ...
        -translate-x-1/2 -translate-y-1/2">
            ...
            <ButtonComponent handleClick={() => {
                if (typeof functionButtonFirst === "function") {
                    functionButtonFirst(paramButtonFirst);
                }
                modalDispatch(modalActions.closeModal());
            }} className="text-main text-sm text-center w-full py-3 font-semibold border-t border-b 
            border-solid border-gray-200 hover:bg-gray-100">{buttonFirst}</ButtonComponent>
            <ButtonComponent handleClick={() => {
                if (typeof functionButtonSecond === "function") {
                    functionButtonSecond(paramButtonSecond);
                }
                modalDispatch(modalActions.closeModal());
            }}
